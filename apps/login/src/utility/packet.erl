...
unpack(Cmd, Bin) ->
	...
	try all_pb:decode_msg(Bin, Name) of
		...
		Class:Reason:Stacktrace ->
			lager:error("~n error in unpack, Stacktrace:~s", [lager:pr_stacktrace(Stacktrace, {Class, Reason})]),
			error
	end.
...
unpack_pb(Name, Bin)->
	try all_pb:decode_msg(Bin, Name) of
		...
		Class:Reason:Stacktrace ->
			lager:error("~n error in unpack_pb, Stacktrace:~s", [lager:pr_stacktrace(Stacktrace, {Class, Reason})]),
			undefined
	end.
